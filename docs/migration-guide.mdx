...
- **`CollectionReference`**:
  - **BREAKING**: Getting a collection parent document via `parent()` has been changed to a getter `parent`. @ehesp confirm
  - **DEPRECATED**: Calling `document()` is deprecated in favor of `doc()`.
...
- **`DocumentReference`**:
  ...
  - **BREAKING**: Getting a document parent collection via `parent()` has been changed to a getter `parent`. @ehesp confirm
...
- **`Transaction`**: @ehesp: confirm method naming (e.g. `set`)
